...
interface OnboardingVisualIdentityLayoutSectionProps {
  layoutRules: string;
  layoutFiles: File[];
  ...
  return (
    <>
      <Card className="p-6 bg-gray-800 border-gray-700">
        ...
        {extractionStatus && extractionVerified === false && !showAdjustMessage && (layoutRules.length > 0 || layoutFiles.length > 0) && (
          <div className="mb-4 p-3 bg-blue-900/20 border border-blue-600/30 rounded-lg">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <CheckCircle className="h-4 w-4 text-blue-400" />
                <span className="text-blue-400 text-sm">Ready to confirm your layout rules?</span>
              </div>
              <Button
                size="sm"
                variant="outline"
        ...
        <div className="space-y-2">
          ...
          <Textarea
            value={layoutRules}
            onChange={(e) => onLayoutRulesChange(e.target.value)}
      ...
      <ClearAllConfirmationDialog
        ...
        title="Clear All Layout Data"
        description="Are you sure you want to clear all layout rules and files? This action cannot be undone."
      />
    </>
  );
};
